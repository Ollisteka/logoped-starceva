  ...
  const mentorshipActivities = [
    {
      ...
      photos: {
        ...
        images: [
          ...
          {
            src: '2.jpg',
            alt: 'Светлана Александровна и студентка УрГПУ занимаются с ребёнком перед зеркалом, у ребёнка высунут язык',
          },
          {
            src: '3.jpg',
            alt: 'Светлана Александровна и студентка УрГПУ занимаются с ребёнком перед зеркалом, студентка и ребёнок улыбаются',
          },
        ],
      },
    },
  ...
  return (
    <BasePage heading="Наставничество">
      {mentorshipActivities.map((activity, index) => (
        <Article key={index} title={activity.title} year={activity.period} description={activity.description}>
          {activity.downloads && <LinkList downloads={activity.downloads} />}

          {activity.photos && <PhotoGallery photos={activity.photos} title={activity.title} />}
        </Article>
      ))}
    </BasePage>
  );
